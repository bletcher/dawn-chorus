...
class SolarModel:
    ...
    def _events(self, d: _date) -> dict:
        # astral returns tz-aware datetimes in the tz we pass in.
        return sun(self._loc.observer, date=d, tzinfo=self.tz)

    def sunrise(self, d: _date) -> datetime:
        return self._events(d)["sunrise"]

    def dawn(self, d: _date) -> datetime:
        return self._events(d)["dawn"]  # civil dawn (6 deg depression)
