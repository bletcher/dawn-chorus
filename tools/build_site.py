...
year + a brushed range on the record strip). Move the selection and every chart recomputes for it:
  * Dawn timeline   - median onset->offset per species, coloured by the proportion of
  * Cumulative call distributions (ECDF) - mean of the period's per-morning curves
  *                   5-min windows with detections
* Times heatmap   - species x solar-minute, proportion of the period's mornings with detections
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* Five bordered pills competed with the title and with each other for a set of actions
     nobody performs while reading. Two quiet links now: documentation in one dialog,
     preferences in the other. Theme moved inside Settings, where a preference belongs, and
     gained a "match the system" state it never had as a toggle. */
  .linkbtn{font:inherit; font-size:13px; background:none; border:0; padding:2px 0; cursor:pointer;
  ...
  section.card{background:var(--surface); border:1px solid var(--line); border-radius:12px; padding:18px 18px 8px; margin-bottom:20px; box-shadow:var(--shadow); position:relative}
  /* How to read the chart, on request. It used to sit above every chart whether you needed
     it or not -- 470 words of method the regular reader had already learned. The one line
     that stays visible is the computed finding, because that one changes with the data. */
  .infobtn{position:absolute; top:15px; right:15px; width:22px; height:22px; padding:0;
    border:1px solid var(--line); border-radius:50%; background:var(--surface); color:var(--muted);
    font:600 12px/1 var(--mono,ui-monospace,SFMono-Regular,Menlo,monospace); cursor:pointer}
  .infobtn:hover, .infobtn[aria-expanded="true"]{border-color:var(--accent); color:var(--accent)}
  .about{position:absolute; top:44px; right:14px; z-index:30;
    width:min(52ch, calc(100% - 28px));
    background:var(--surface); border:1px solid var(--line); border-radius:10px;
    box-shadow:0 12px 34px rgba(9,13,20,.28); padding:13px 15px}
  ...
  thead th{text-align:right; color:var(--muted); font-weight:600; padding:7px 10px; border-bottom:1px solid var(--line); position:sticky; top:0; background:var(--surface);
    white-space:normal; max-width:16ch; line-height:1.3}
  ...
  footer .station{margin:0 0 10px; font-family:var(--mono,ui-monospace,SFMono-Regular,Menlo,monospace);
  ...
  /* ── Small screens ────────────────────────────────────────────────────────────────
     A scope bar three rows deep is most of a phone screen, and sticking it leaves almost
     nothing to scroll. It scrolls away here; the rung headers already go static at 900px. */
  @media (max-width:760px){
    .wrap{padding:18px 14px 48px}
    .scopebar{position:static; padding:11px 12px; gap:8px}
    .scopebar .eyebrow{min-width:0}
    .selrow{margin-left:0; padding-left:11px}
    .scoperow{gap:9px 12px}
    .strip{height:52px}
    #periodLabel{font-size:15px}
    .masthead{gap:10px}
    .about{position:static; width:auto; margin:0 0 12px; box-shadow:none; background:var(--scope)}
    .infobtn{position:static; margin-left:auto}
    .card > h2{gap:7px}
  }
  /* Fingers are not mice: every tap target in the chrome gets real size on touch. */
  @media (pointer:coarse){
    .infobtn{width:30px; height:30px}
    .pstep{padding:8px 13px; font-size:16px}
    .metseg button{padding:8px 14px}
    .linkbtn{padding:7px 0}
    .mselbtn{padding:9px 13px}
    .msellist label{padding:9px 6px}
  }
</style>
</head>
<body>
<div class="wrap">
  <header class="masthead">
    ...
    <div class="hbtns">
      <button class="linkbtn" id="guideBtn" aria-haspopup="dialog">Guide</button>
      <button class="linkbtn" id="setBtn" aria-haspopup="dialog">Settings</button>
    </div>
  </header>
  ...
  <section class="hero" id="hero">
    <div class="herohead">
      ...
      <span class="metseg" id="heroMode" role="group" aria-label="Detail">
        ...
        <button type="button" data-mode="detail" aria-pressed="false">Detail</button>
      </span>
    </div>
  ...
  <div class="scopebar">
    ...
    <div class="scoperow floorrow">
      ...
      <span class="scopehint">needed for an onset</span>
    </div>
  </div>
  ...
  <section class="rung" data-rung="selection">
    ...
    <div class="rbody">
      <section class="card" data-card="period">
        ...
        <div class="about" hidden><p>Species in the selection ranked by detections &mdash; the same set as <em>Who sings when</em>. The axis is linear, so bar length is directly comparable, which is why the tail looks thin. Counts are detections, not birds: a species that repeats itself outranks one that calls once from three directions.</p></div>
      ...
      <section class="card" data-card="ecdf">
        ...
        <div class="about" hidden><p>The empirical CDF <em>F(t)</em> &mdash; share of a species' detections that have occurred by each minute &mdash; averaged across the selection's mornings, each morning a replicate. Onset reads where a curve crosses 0.05, median song-time at 0.5, offset at 0.95.</p></div>
        <div class="controls"><div class="msel" id="ecdfSel"></div></div>
        <div class="plot" id="chart-ecdf"></div>
      </section>
      <section class="card" data-card="heat">
        <h2 class="display">Proportion of morning times with calls</h2>
        <button type="button" class="infobtn" aria-expanded="false" aria-label="How to read this chart" title="How to read this chart">i</button>
        <div class="about" hidden><p>Species &times; solar-minute. Colour is the proportion of mornings with detections &mdash; of the mornings in the selection, how many held a detection for that species in that 5-minute window. Not the same measure as the table's <em>proportion of 5-min windows with detections</em>, which counts windows within a single morning. Only species heard in the selection get a row. This is presence, not phenology, so it lists a few species that <em>Who sings when</em> leaves out &mdash; ones heard too sparsely on any single morning to earn an onset.</p></div>
      ...
      <section class="card" data-card="table">
        ...
        <div class="about" hidden><p>Aggregated over the selection: mornings present, total detections, median onset, offset, span and peak, and the proportion of 5-min windows with detections. Hover a column heading for its definition; click a species to hear it.</p></div>
  ...
  <section class="rung" data-rung="season">
    ...
    <div class="rbody">
      <section class="card" data-card="trend">
        ...
        <div class="about" hidden><p>The record rolled up by week, month or year &mdash; smoothing the morning-to-morning noise the strip at the top shows in full. Ignores the selection; highlighted bars are the periods it touches, and clicking one scopes to it. Counts follow the <span class="tag">detection floor</span>.</p></div>
        <div class="controls">
      ...
      <section class="card" data-card="season">
        ...
        <div class="about" hidden><p>Each dot is one species on one morning: when it started, against the date. Trend lines appear for species with &ge;4 mornings. Ignores the selection &mdash; a season is what it measures.</p></div>
...
<div class="modal" id="setModal" hidden>
  <div class="dialog" role="dialog" aria-modal="true" aria-label="Settings">
    ...
    <div class="setrow">
      <b>Theme</b>
      <label><input type="radio" name="theme" value="system" checked>
        <span>Match the system
          <small>Follows your operating system's light or dark setting, and changes with it.</small></span></label>
...
<div class="modal" id="guide" hidden>
  <div class="dialog" role="dialog" aria-modal="true" aria-label="User's guide">
    ...
    <ul>
      ...
      <li><b>Proportion of morning times with calls</b> — species &times; 5-minute bin; colour is the proportion of the period's mornings
    ...
    <h2 class="display" style="margin-top:26px">Adding data</h2>
...
function updateAudioCard(S){
  ...
  if(lead && one) lead.innerHTML =
    `Click any chart or a species name to hear it. <strong>‹ ›</strong> steps through calls, ▶ plays.`;
}
...
function W(el){ return Math.max(220, el.clientWidth || 900); }
...
// The masthead used to carry "14 mornings · 42.54, -72.53 · America/New_York". The count is
// in the strip and every rung header; the coordinates and timezone are reference data nobody
// reads twice, so they moved to the footer with the rest of the method.
function stationLine(){ const m=meta;
  const loc=(m.lat!=null&&m.lon!=null)?`${m.lat.toFixed(4)}, ${m.lon.toFixed(4)}`:"";
  return [loc, m.tz, (m.recorders||[]).join(", ")].filter(Boolean).join(" &middot; "); }
...
/* This chart owns its own grain, and the finest it offers is Week.
   It used to follow the Snap control, which meant that at Snap = Morning it drew exactly the
   context strip, one screen below the context strip -- the same series twice, with the larger
   copy adding nothing but axes. Rolling up is the one thing the strip refuses to do (context
   must stay at full resolution), so that is what this does and all it does. While the record
   is too short for the roll-up to say anything, autoThin folds the card away. */
let TREND_GRAIN=(()=>{ const v=localStorage.getItem("dc_trend_grain");
  return ["week","month","year"].includes(v) ? v : "week"; })();
function renderTrend(){
  const el=document.getElementById("chart-trend"); el.innerHTML="";
  const level=TREND_GRAIN, keys=periodsFor(level);
  const rows=trendRows(level, keys), metric=TREND_METRIC;
  ...
  const g=document.getElementById("trendGrainLabel");
  if(g) g.textContent = `by ${PERIOD_WORD[level]||level}`;
  autoThin("trend", rows.length, PERIOD_WORD[level]||level);
...
const SETTINGS=Object.assign({xmode:"dawn", autoCollapse:true, theme:"system"},
...
function autoThin(key, n, noun){
  ...
  // `noun` for charts that are not scoped: "only 4 weeks on record" is the truth for the
  // roll-up, where "in this period" would be plainly wrong.
  const what = noun ? `only ${n} ${noun}${n===1?"":"s"} on record`
                    : (n ? `only ${n} point${n===1?"":"s"} in this period` : "no data in this period");
  if(h){ if(thin){ if(!why){ why=document.createElement("span"); why.className="why"; h.appendChild(why); }
                   why.textContent = n ? what : (noun ? "nothing on record yet" : "no data in this period"); }
...
let HERO_MODE = localStorage.getItem("dc_hero_mode")==="detail" ? "detail" : "picture";
function renderTimeline(S){
  ...
  el.append(Plot.plot({ width:W(el), height:names.length*(pic?24:26)+(pic?58:80),
    ...
    color: pic ? undefined
      : {type:"linear", domain:[0,1], range:[css("--seq-lo"),css("--seq-hi")], legend:true, label:"median proportion of 5-min windows with detections"},
    marks:[
...
function renderHeat(S){
  ...
  el.append(Plot.plot({ width:W(el), height:names.length*20+80, marginLeft:ML(el,true), marginRight:26, style:plotStyle(),
    ...
    color:{type:"linear", domain:[0,1], range:[css("--seq-lo"),css("--seq-hi")], legend:true, label:"proportion of mornings with detections"},
    marks:[
...
const COLDESC = {
  ...
  "proportion of 5-min windows with detections": "0 to 1, over the 5-minute windows between this species' first and last song of the morning. 1 means every window held a detection -- it sang right through; 0.3 means long gaps. Median across the selection's mornings."
};
...
function renderTable(S){
  ...
  const head=["Species","mornings","n","onset","offset","span","peak","proportion of 5-min windows with detections"];
...
function renderFindings(S){
  ...
  // Reports on the roll-up, so it follows the roll-up's grain, not the selection's.
  const tr=trendRows(TREND_GRAIN, periodsFor(TREND_GRAIN)), M=TREND_METRIC,
        word=PERIOD_WORD[TREND_GRAIN]||TREND_GRAIN;
  if(tr.length){
...
(()=>{ const g=document.getElementById("trendGrain"); if(!g) return;
  g.value=TREND_GRAIN;
  g.onchange=()=>{ TREND_GRAIN=g.value; localStorage.setItem("dc_trend_grain", TREND_GRAIN);
    renderTrend(); renderFindings(scopedDays()); };
})();
...
// nested inside another control's hit area is a trap however carefully you stop propagation.
function closeAbouts(except){
  document.querySelectorAll(".card .about").forEach(a=>{
    if(a===except) return;
    a.hidden=true;
    const b=a.closest(".card").querySelector(".infobtn");
...
document.addEventListener("click", e=>{
  ...
  if(btn){
    const card=btn.closest(".card"), about=card && card.querySelector(".about");
    if(!about) return;
    const show=about.hidden;
    closeAbouts(show ? about : null);
    about.hidden=!show;
    btn.setAttribute("aria-expanded", String(show));
    return;
  }
  if(!e.target.closest(".about")) closeAbouts(null);
});
addEventListener("keydown", e=>{ if(e.key==="Escape") closeAbouts(null); });
...
// Three states, not a blind toggle: "system" is the default and the only one that keeps
// following the OS, so it has to be expressible rather than something you fall out of.
function applyTheme(){
...
document.querySelectorAll('input[name="theme"]').forEach(r=> r.addEventListener("change", ()=>{
  if(!r.checked) return;
  SETTINGS.theme=r.value; saveSettings(); applyTheme(); renderAll();
}));
(function(){
...
renderFoot(); buildChips(); syncTrendMetric(); syncHeroMode(); rebuildPeriods();
}
...
(function(){
  ...
  fetch("/api/state", {signal: ctl.signal})
    ...
    .then(() => {
      ...
      a.innerHTML = '<button class="linkbtn">Control panel</button>';
      hb.appendChild(a);
    })
