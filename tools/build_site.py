  ...
  <section class="chapter" id="ch-overview">
    ...
    <div class="chbody">
      <section class="card" data-card="overview">
        ...
        <p class="lead">Every species detected in the <span class="tag">scoped period</span>, ranked by
          number of detections. The axis is linear, so bar length is directly comparable &mdash; which is
          why the tail looks thin: a few species really do account for most of what the recorder hears.
          Counts are detections above the confidence floor, not individual birds, so a species that
          repeats itself outranks one that calls once from three directions.</p>
...
function renderOverview(S){
  ...
  const rows=Object.keys(g).map(name=>({name, n:g[name].reduce((s,r)=>s+r.n,0),
                                        mornings:g[name].length}))
    .filter(d=>d.n>0)
    .sort((a,b)=> b.n-a.n || a.name.localeCompare(b.name));   // ties resolve stably, not randomly
  if(!rows.length){ el.innerHTML='<p class="empty">No detections in this period.</p>'; return; }
...
function renderFindings(S){
  ...
  const cnt=Object.keys(g).map(n=>({n, det:g[n].reduce((s,r)=>s+r.n,0)}))
                          .filter(d=>d.det>0).sort((a,b)=>b.det-a.det);
  if(cnt.length){
