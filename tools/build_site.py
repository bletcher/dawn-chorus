...
year + a brushed range on the record strip). Move the selection and every chart recomputes for it:
  * Dawn timeline   - median onset->offset per species, coloured by median continuity
  * Cumulative call distributions (ECDF) - mean of the period's per-morning curves
  * Regularity heatmap - species x solar-minute, share of the period's mornings present
  ...
  <section class="rung" data-rung="selection">
    ...
    <div class="rbody">
      ...
      <section class="card" data-card="heat">
        <h2 class="display">Regularity through the morning</h2>
        <button type="button" class="infobtn" aria-expanded="false" aria-label="How to read this chart" title="How to read this chart">i</button>
        <div class="about" hidden><p>Species &times; solar-minute. Colour is the share of the selection's mornings on which a species was detected in each 5-minute bin &mdash; how <em>reliably</em> it is there at that time, which is a different measure from the continuity in the table (that one is within a single morning). Only species heard in the selection get a row. This is presence, not phenology, so it lists a few species that <em>Who sings when</em> leaves out &mdash; ones heard too sparsely on any single morning to earn an onset.</p></div>
      ...
      <section class="card" data-card="table">
        ...
        <div class="about" hidden><p>Aggregated over the selection: mornings present, total detections, and median onset/offset/span/peak/continuity. Hover a column heading for its definition; click a species to hear it.</p></div>
...
<div class="modal" id="guide" hidden>
  <div class="dialog" role="dialog" aria-modal="true" aria-label="User's guide">
    ...
    <ul>
      ...
      <li><b>Regularity</b> — species &times; 5-minute bin; colour is the share of the period's mornings
...
function renderTimeline(S){
  ...
  el.append(Plot.plot({ width:W(el), height:names.length*(pic?24:26)+(pic?58:80),
    ...
    color: pic ? undefined
      : {type:"linear", domain:[0,1], range:[css("--seq-lo"),css("--seq-hi")], legend:true, label:"median continuity"},
    marks:[
...
function renderHeat(S){
  ...
  el.append(Plot.plot({ width:W(el), height:names.length*20+80, marginLeft:ML(el,true), marginRight:26, style:plotStyle(),
    ...
    color:{type:"linear", domain:[0,1], range:[css("--seq-lo"),css("--seq-hi")], legend:true, label:"share of mornings present"},
    marks:[
...
const COLDESC = {
  ...
  "continuity": "0 to 1: of the 5-minute bins between this species' first and last song, the fraction that held at least one detection. 1 means it sang right through; 0.3 means long gaps."
};
...
function renderTable(S){
  ...
  const head=["Species","mornings","n","onset","offset","span","peak","continuity"];
