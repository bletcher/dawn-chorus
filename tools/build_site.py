...
function drawLabels(){                                     // BirdNET 3s detections boxed + named over the clip
  ...
  if(cnt) cnt.textContent = inClip ? `${rows.length} box${rows.length===1?'':'es'} · ${new Set(rows.map(r=>r.si)).size} species of ${inClip} detection${inClip===1?'':'s'} in view` : 'no detections in view';
