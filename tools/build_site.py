...
function renderTimeline(S){
  ...
  el.append(Plot.plot({ width:W(el), height:names.length*26+80, marginLeft:172, marginRight:26, style:plotStyle(),
    x:xAxis({grid:true}), y:{domain:names, label:null},
...
function renderHeat(S){
  ...
  el.append(Plot.plot({ width:W(el), height:names.length*20+80, marginLeft:172, marginRight:26, style:plotStyle(),
    x:xAxis({grid:false}), y:{domain:names, label:null},
...
function renderSeason(){
  ...
  if(!pts.length){ el.innerHTML='<p class="empty">No species selected, or none has a defined onset yet.</p>'; return; }
  ...
  const marks=[
    Plot.ruleY([xv(0)], {stroke:css("--dawn"), strokeWidth:1.5, strokeDasharray:"4,3"}),
    Plot.dot(pts, {x:"d", y:d=>xv(d.y), fill:"name", r:3.6, fillOpacity:.85,
      stroke:css("--surface"), strokeWidth:.6, tip:true,
      title:d=>`${d.name}\n${d.date}\nonset ${xTitle(d.y)}\n${d.n} detections`}),
  ];
  if(fitPts.length && Plot.linearRegressionY)
    marks.push(Plot.linearRegressionY(fitPts, {x:"d", y:d=>xv(d.y), stroke:"name", z:"name", strokeWidth:1.5, ci:0}));
  el.append(Plot.plot({ width:W(el), height:420, marginLeft:56, marginRight:20, style:plotStyle(),
...
function applySettings(){
  document.querySelectorAll('input[name="xmode"]').forEach(r=>{ r.checked = r.value===SETTINGS.xmode; });
...
document.querySelectorAll('input[name="xmode"]').forEach(r=> r.addEventListener("change", ()=>{
  if(!r.checked) return;
  SETTINGS.xmode=r.value; saveSettings(); renderAll();
}));
