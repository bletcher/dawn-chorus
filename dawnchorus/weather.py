...
def fetch_hourly(latitude, longitude, start_date, end_date, tz,
    ...
    if cache_path and Path(cache_path).exists():
        ...
        # A cache written for an earlier/shorter date range silently under-covers
        # once the station DB grows. Warn so those mornings aren't quietly blank.
        have0, have1 = df.index.min().date(), df.index.max().date()
        want0, want1 = pd.Timestamp(start_date).date(), pd.Timestamp(end_date).date()
        if have0 > want0 or have1 < want1:
            warnings.warn(
                f"weather cache {cache_path} covers {have0}..{have1} but "
                f"{want0}..{want1} was requested; mornings outside the cached "
                f"range will have no weather. Delete the cache to refetch.",
                stacklevel=2,
            )
        return df
...
def morning_weather(hourly: pd.DataFrame, solar, dates, config=None) -> pd.DataFrame:
    ...
    for d in pd.Index(pd.unique(pd.to_datetime(list(dates)).date)):
        ...
        if anchor_dt is None:  # polar day/night: no anchor, skip this morning
            continue
